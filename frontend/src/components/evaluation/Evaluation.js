...
const Evaluation = () => {
    ...
    const getAlldataevaluation = async () => {
        try {
            const response = await axios.get(`${url}getAlldataevaluation/${ids}`);
    ...
    const etape1 = (placement) => {
        ...
        notification.info({
            message: `Notification`,
            description: "Vous ne pouvez pas évaler vous même.",
            placement,
        });
        return;
                        ...
                        <tbody>
                            ...
                            <td style={{ padding: '10px', width: '20%' }}>
                                ...
                                <th style={{ padding: '10px', width: '20%',textAlign:'center',alignItems:'center' }}>
                                    <AutoComplete
                                        style={{ width: '100%' }}
                                        placeholder="e-mail d'évaluateur"
                                        onChange={handleSearch}
                                        value={email}
                                        
                                        dataSource={emails.filter(email => email.includes(searchValue))}
                                        loading={loading}
                                        allowClear
                                    />
     ...
     
        {
            ...
            content: (
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', flexDirection: 'column' }}>
                    ...
                    {/* <Title level={5}>* Seul l'évaluateur que vous avez mentionné dans la première étape peut valider votre évaluation</Title> */}
