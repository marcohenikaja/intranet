...
const Af = () => {
    ...
    const recupenbraf = async () => {
        try {
            const recupeservig = await axios.get(`${url}recupenbraf`);
            console.log((recupeservig.data));
            setnbraf(recupeservig.data.count)
        } catch (error) {
            console.error("Error uploading image:", error);
        }
    }
    ...
    const confsupprimeraf = async (e) => {
        try {
            const confsupprimeraf = await axios.delete(`${url}confsupprimeraf/${e}`)
            setAlertsup(true)
    ...
    const ajouteraf = async () => {
        ...
        try {
            ...
            const ajouteraf = await axios.post(`${url}ajouteraf`, { nom, prenom, poste, tel, mail, desc, imageUrlToSend, });
            ...
            <div className="titleHolder">
                ...
                {statut === 'Super admin' || statut === 'Admin' ? (
                  ...
                  <Row gutter={16} align="middle" justify="center">
                      <Col span={3}>
                      <Statistic title="Personnes" value={nbraf} formatter={formatter} />
                      </Col> 
                  </Row>
