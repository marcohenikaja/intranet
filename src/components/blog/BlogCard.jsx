...
const BlogCard = () => {
  ...
  const makamedia = async () => {
    try {
      const medias = await axios.get(`${url}makamedia`).then((result) => {
  ...
  const manovaarticle = async () => {
    const data = { titre: titreupd, description: descupd };
    try {
    ...
    <>
        ...
        if (fileExtension === 'jpeg' || fileExtension === 'jpg' || fileExtension === 'png' || fileExtension === 'mp4' || fileExtension === 'avi' || fileExtension === 'mov') {
          return (
            <Fade left key={val.id}>
              <div className='items shadow' style={{ width: '500px',height: '700px'}} >
                <div className='img'>
                  { fileExtension === 'jpeg' || fileExtension === 'jpg' || fileExtension === 'png' ? (
                      <img
                      src={url + val.imageUrl}
                      alt=''
                      style={{ width: '500px', height: isImageEnlarged ? '700px' : '350px' }}
                     
                    />
                  ) : (
                    <video controls className='video-element' style={{ width: '500px' ,height: '500px'}}>
                ...
                <div className='text'>
                  ...
                  <h1>{val.titre}</h1>
                  <p style={{ maxHeight: '150px', overflow: 'auto' }}>{val.description} </p>
      ...
      <Modal open={ouvrir} onClose={() => setOuvrir(false)}>
        <div style={{ textAlign: 'center', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', minWidth: '150px' }}>
          ...
          <TextField id="outlined-basic" value={descupd} onChange={(e) => setDescupd(e.target.value)} label="Description" variant="outlined" sx={{ mb: 2 }} /><br />
