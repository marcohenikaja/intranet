   ...
   const [idsuppr,setIdsuppr] =useState("")
    ...
    const ajoutermark = async () => {
        ...
        try {
            ...
            const ajoutermark = await axios.post(`${url}ajoutermark`, { nom, prenom, poste, desc, imageUrlToSend, });
    ...
    const recupemark= async () => {

        try {
            const recupemark = await axios.get(`${url}recupemark`);
            setMark(recupemark.data)
        } catch (error) {
            console.error("Error uploading image:", error);
        }
    }
    ...
    const confsupprimermark=async()=>{
        try {
           const confsupprimer =await axios.delete(`${url}confsupprimermark/${idsuppr}`)
           setAlertsup(true)
    ...
    return (
        <>
            <section className='back10'></section>
            <div style={{ fontSize: '80px', color: 'white', textAlign: 'center', marginTop: '200px' }}>
                <h1 style={{ fontSize: '80px', color: '#FC1E9F' }}>Marketing et Communication</h1>
            </div>
            ...
            <Modal open={open} onClose={handleClose} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', textAlign: 'center', }} >
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'background.paper', boxShadow: 4, p: 4, maxWidth: '400px', }}>
                    ...
                    <Button variant="contained" color="primary" style={{ backgroundColor: ' #329358 ' }} sx={{ mt: 2 }} onClick={ajoutermark}>
                        Enregistrer
                    </Button>
                </Box>
            </Modal>
            ...
            <Modal open={ouvrir} onClose={() => setOuvrir(false)}>
                    <div style={{ textAlign: 'center', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', minWidth: '150px' }}>
                        ...
                        <Button  variant="contained" onClick={updatedatamark}  color="success" style={{ marginTop: '20px', backgroundColor: ' #329358 ' }}>Enregistrer</Button>
                    </div>
                </Modal>
                ...
                <Dialog  open={sokafana} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description"  >
                        ...
                        <DialogActions>
                            ...
                            <Button  color="primary" autoFocus onClick={confsupprimermark}>
                            Confirmer
                            </Button>
                        </DialogActions>
                </Dialog>
