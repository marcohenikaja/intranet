...
const Servicegenerau = () => {
    ...
    const confsupprimerrh = async () => {
        try {
            const confsupprimer = await axios.delete(`${url}confsupprimerrh/${idsuppr}`)
            setAlertsup(true)
    ...
    const ajouterrh = async () => {
        ...
        try {
            ...
            const ajouterrh = await axios.post(`${url}ajouterrh`, { nom, prenom, poste, desc, imageUrlToSend, });
    ...
    return (
        <>
            <section className='back11'></section>
            <div style={{ fontSize: '80px', color: 'white', textAlign: 'center', marginTop: '200px' }}>
                <h1 style={{ fontSize: '80px', color: 'white' }}>Ressources Humaines</h1>
            ...
            <Modal open={open} onClose={handleClose} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', textAlign: 'center', }} >
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'background.paper', boxShadow: 4, p: 4, maxWidth: '400px', }}>
                    ...
                    <Button variant="contained" color="primary" style={{ backgroundColor: ' #329358 ' }} sx={{ mt: 2 }} onClick={ajouterrh}>
                        Enregistrer
                    </Button>
                </Box>
            </Modal>
            ...
            <Modal open={ouvrir} onClose={() => setOuvrir(false)}>
                <div style={{ textAlign: 'center', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', minWidth: '150px' }}>
                    ...
                    <Button variant="contained" onClick={updatedatarh} color="success" style={{ marginTop: '20px', backgroundColor: ' #329358 ' }}>Enregistrer</Button>
                </div>
            </Modal>
            ...
            <Dialog open={sokafana} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description"  >
                ...
                <DialogActions>
                    ...
                    <Button color="primary" autoFocus onClick={confsupprimerrh}>
                        Confirmer
                    </Button>
                </DialogActions>
            </Dialog>
