...
const Servicegenerau = () => {
    ...
    const ajouterservig = async () => {
        ...
        try {
            const responseUpload = await axios.post(`${url}UploadImageserviceg`, formData, {
                headers: {
            ...
            const ajouterdir = await axios.post(`${url}ajouterservig`, { nom, prenom, poste, desc, imageUrlToSend, });
    ...
    const confsupprimerservg = async () => {
        try {
            const confsupprimer = await axios.delete(`${url}confsupprimerservg/${idsuppr}`)
            setAlertsup(true)
    ...
    return (
        <>
            ...
            <div style={{ fontSize: '80px', color: 'white', textAlign: 'center', marginTop: '200px' }}>
                <h1 style={{ fontSize: '80px', color: 'white' }}>Services généraux </h1>
            </div>
...
{/* 
            ...
            <Modal open={open} onClose={handleClose} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', textAlign: 'center', }} >
                <Box sx={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', bgcolor: 'background.paper', boxShadow: 4, p: 4, maxWidth: '400px', }}>
                    ...
                    <Button variant="contained" color="primary" style={{ backgroundColor: ' #329358 ' }} sx={{ mt: 2 }} onClick={ajouterservig}>
                        Enregistrer
                    </Button>
                </Box>
            </Modal>
            ...
            <Modal open={ouvrir} onClose={() => setOuvrir(false)}>
                <div style={{ textAlign: 'center', position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', backgroundColor: 'white', padding: '20px', minWidth: '150px' }}>
                    ...
                    <Button variant="contained" onClick={updatedataservg} color="success" style={{ marginTop: '20px', backgroundColor: ' #329358 ' }}>Enregistrer</Button>
                </div>
            </Modal>
            ...
            <Dialog open={sokafana} onClose={handleClose} aria-labelledby="alert-dialog-title" aria-describedby="alert-dialog-description"  >
                ...
                <DialogActions>
                    ...
                    <Button color="primary" autoFocus onClick={confsupprimerservg}>
                        Confirmer
                    </Button>
                </DialogActions>
            </Dialog>
